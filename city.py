...
def access_to_closest_streets(g1: OsmnxGraph, g2: mt.MetroGraph) -> list:
    """ Returns a list where every element is in this format: (node1, node2, distance from node1 to node2). 
    Node1 is always an access, and node2 is it's closest street from the g1 graph. In this way later we only 
    have to iterate this list and create edges from node1 to node2, adding the distance to the attributes """
